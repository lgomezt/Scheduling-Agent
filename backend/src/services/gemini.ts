...
export const pdfToScenarios = async (
  ...
  const parsed = scenariosResponseSchema.parse(parseJsonResponse(text));
...
export const proposeChoice = async (input: SchedulerInput): Promise<AgentProposal> => {
  ...
  return proposalSchema.parse(parseJsonResponse(result.response.text()));
};
...
export const generateStudyInitialProfile = async (input: {
  ...
  const result = await model.generateContent([
    {
      text:
        "Here are the questionnaire responses available to this model condition. Produce the markdown profile now.\n\n" +
...
export const generateStudyFinalProfile = async (input: {
  ...
  const result = await model.generateContent([
    {
      text:
        "Here is the full end-of-study evidence available to this model condition. Produce the final markdown profile now.\n\n" +
        JSON.stringify(input.payload, null, 2),
    },
  ]);
  const raw = result.response.text();
